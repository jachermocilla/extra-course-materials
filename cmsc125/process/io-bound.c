...
void do_some_work() {
    // Interactive processes have bursty, event-driven behavior
    // They do short bursts of work followed by waiting for events
    
    static int work_cycle = 0;
    work_cycle++;
    
    // Simulate different types of interactive workloads
    int workload_type = rand() % 100;
    
    if (workload_type < 40) {
        // 40% - Light UI update work (very short CPU burst)
        volatile long counter = 0;
        for (int i = 0; i < 50000; i++) {
            counter += i * (i & 1);  // Some computation
        }
        
        // Short sleep simulating waiting for next UI event
        usleep(rand() % 20000 + 5000);  // 5-25ms
        
    } else if (workload_type < 70) {
        // 30% - User input processing (moderate CPU burst)
        ...
        // Brief I/O simulation (file access, network)
        usleep(rand() % 10000 + 2000);  // 2-12ms
        
    } else if (workload_type < 85) {
        // 15% - Background processing while interactive
        // Longer CPU work but with voluntary yields
        volatile double calculation = 1.0;
        for (int i = 0; i < 100000; i++) {
            calculation = calculation * 1.001 + 0.001;
    ...
    } else if (workload_type < 95) {
        // 10% - I/O heavy operation (database query, file read)
        // Very little CPU, mostly waiting
        ...
        // 5% - Intensive burst (complex calculation, rendering)
        volatile long complex_work = 0;
        int burst_size = rand() % 500000 + 100000;  // 100k-600k iterations
        
        for (int i = 0; i < burst_size; i++) {
            complex_work += i * i;
            complex_work ^= (complex_work >> 16);
            
            // Even during intensive work, yield occasionally
            if (i % 50000 == 0) {
    ...
    // Periodic longer pause to simulate user "think time"
    // Interactive processes often wait for user input
    if (work_cycle % 20 == 0) {
        // Every 20th cycle, simulate waiting for user interaction
        usleep(rand() % 100000 + 50000);  // 50-150ms pause
    }
    
    // Occasionally simulate a completely idle period
    // (waiting for user to move mouse, type, etc.)
    if (rand() % 200 == 0) {
        usleep(rand() % 500000 + 100000);  // 100-600ms idle
    }
}
